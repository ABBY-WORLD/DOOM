...
function bindEvents() {
  ...
  radioRandomButton.addEventListener("click", () => {
    markActivity();
    switchRadioStation();
  });
  ...
  feedDeck.addEventListener("scroll", () => {
    updateActiveFeedFromScroll();
    markActivity();
  });
  classicFeed.addEventListener("scroll", markActivity, { passive: true });
  arenaFeed.addEventListener("scroll", markActivity, { passive: true });
  ...
  feedDeck.addEventListener("click", handleDeckClick);
...
function handlePointerEnd(event) {
  ...
  if (Math.abs(deltaX) < 54 || Math.abs(deltaX) < Math.abs(deltaY) * 1.35) {
    if (Math.hypot(deltaX, deltaY) < 16) {
      toggleInterface();
      suppressDeckClickUntil = Date.now() + 450;
    }
    return;
  }
...
function handleDeckClick(event) {
  markActivity();
  if (event.target.closest("button, a") || boardPanel.classList.contains("is-open")) return;
...
function revealEraSwitch() {
  appShell.classList.remove("is-era-muted");
  if (eraFadeTimer) {
    window.clearTimeout(eraFadeTimer);
  }

  eraFadeTimer = window.setTimeout(() => {
    appShell.classList.add("is-era-muted");
    eraFadeTimer = null;
  }, 2800);
}
...
function resetIdleScrollTimer() {
  clearIdleScrollTimer();
  if (!hasStartedExperience || boardPanel.classList.contains("is-open")) return;

  idleScrollTimer = window.setTimeout(scrollToNextArtwork, 15000);
}

function clearIdleScrollTimer() {
  if (idleScrollTimer) {
    window.clearTimeout(idleScrollTimer);
    idleScrollTimer = null;
  }
}

function scrollToNextArtwork() {
  if (!hasStartedExperience || boardPanel.classList.contains("is-open")) return;
...
function setArenaStatus() {
  if (activeFeedName === "arena") {
    setStatus(`${feedState.arena.items.length} Are.na images`);
    return;
  }
